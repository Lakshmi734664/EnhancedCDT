...
public class RecordIdentifer {
	...
	public Element getMatchingUniqueElement(boolean isCompareReport) throws Exception {
		...
		if (primaryKeyValue != null && !primaryKeyValue.trim().isEmpty()) {
			outEle = getElementUsingXpath(new StringBuffer("//[@" + primaryKeyName + "='" + primaryKeyValue + "']"),
					isCompareReport, doc);
		}
		...
		if (!tblRecordIdentifierMap.isEmpty()) {
			...
			tblRecordIdentifierMap.forEach((k, v) -> xpathExpr
					.append("[@*[translate(name(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')='" + k
							+ "']='" + v + "']"));
			...
			if (configValue != null && !configValue.trim().isEmpty()) {
				...
				for (String identifier : identifiers) {
					...
					if (identifier.contains("|")) {
						...
						addRecordIdentifier(identifier.toLowerCase(), elemToMatch.getAttribute(identifier),
								tblRecordIdentifierMap);
					}
				}
			}
		}
		addRecordIdentifier(CDTConstants.organizationCode.toLowerCase(),
				elemToMatch.getAttribute(CDTConstants.organizationCode), tblRecordIdentifierMap);
		addRecordIdentifier(CDTConstants.processTypeKey.toLowerCase(),
				elemToMatch.getAttribute(CDTConstants.processTypeKey), tblRecordIdentifierMap);
	...
	private Element getElementUsingXpath(StringBuffer xpathExpr, boolean isCompareReport, Document doc)
			throws Exception {
		Element outEle = null;
		if (isCompareReport) {
		...
		NodeList nodeList = (NodeList) EnhancedCDTMain.xPath.compile(xpathExpr.toString()).evaluate(doc,
