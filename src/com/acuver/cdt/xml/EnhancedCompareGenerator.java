...
public class EnhancedCompareGenerator {
    ...
    public void createEnhancedCompare(String updateAttrName, Element updateElement) throws Exception {
        ...
        Document oldAttrValueDoc = EnhancedCDTMain.factory.newDocumentBuilder().parse(new InputSource(new StringReader(subXmlOldAttrValue)));

        Document newAttrValueDoc = EnhancedCDTMain.factory.newDocumentBuilder().parse(new InputSource(new StringReader(subXmlUpdateAttrValue)));
        ...
        //       System.out.println("Old :" + CDTHelper.convertDocumentToString(oldAttrValueDoc));
        //       System.out.println("New :" + CDTHelper.convertDocumentToString(newAttrValueDoc));
    ...
    private void diffNode(Element oldAttrValueEle, Element newAttrValueEle) throws Exception {
        ...
        if (oldAttrValueEle.getNodeName().equalsIgnoreCase("Points")) {
            if (oldAttrValueEle.getParentNode() != null) oldAttrValueEle.getParentNode().removeChild(oldAttrValueEle);
            return;
        }
        ...
        for (int i = 0; i < oldAttributes.getLength(); i++) {
            ...
            if (newAttributes.getNamedItem(oldAttrName) != null) {
                ...
                if (!oldAttrValue.equals(newAttrValue)) {
                    if (oldAttrValue.matches("\\d+") && oldAttrValue.length() > 5 && newAttrValue.matches("\\d+") && newAttrValue.length() > 5) {
                    }
                    else
                        count++;
                }
    ...
    private Element diffAttributes(Element expectedElement, Element actualElement) throws Exception {
        ...
        String expression = CDTConstants.forwardSlash + expectedElement.getNodeName();
        NodeList actualElementNodes = (NodeList) EnhancedCDTMain.xPath.compile(expression).evaluate(actualElement, XPathConstants.NODESET);
